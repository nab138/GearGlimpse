...
import { IonCard, IonIcon, IonItem, IonReorder, IonReorderGroup, IonTitle } from "@ionic/react";
    ...
    <IonCard className="bulk-select-group bulk-select-selected">
        ...
        >
            ...
            {props.selectedItems.map((item) => (
                <IonItem key={item} >
                    <IonIcon slot="start" icon={removeCircle} className={(props.minItems && props.selectedItems.length <= props.minItems) ? "": "remove-icon"} onClick={() => {
        ...
        <IonCard className="bulk-select-unselected bulk-select-group ion-no-padding">
            {props.items.filter((item) => !props.selectedItems.includes(item)).map((item) => (
                <IonItem key={item} >
                    <IonIcon slot="start" icon={addCircle} className={props.maxItems && props.selectedItems.length >= props.maxItems ? "": "add-icon"} onClick={() => {
